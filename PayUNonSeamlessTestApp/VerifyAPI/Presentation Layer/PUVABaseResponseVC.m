...
- (IBAction)unwindToViewControllerForDeleteSC:(UIStoryboardSegue *)unwindSegue{
    if ([[_tableVC segueIdentifier] isEqual:segueIdentiiferVar2]) {
        if (_tableVC.tappedOnUITableViewCell) {
            ...
            PayUModelStoredCard *storedCardObj = [PayUModelStoredCard new];
            storedCardObj = [self.paymentRelatedDetail.storedCardArray objectAtIndex:_tableVC.selectedIndex];
    ...
    if ([[_tableVC segueIdentifier] isEqual:segueIdentiiferVar1]) {
        ...
        if (_tableVC.tappedOnUITableViewCell) {
            ...
            if ([_tableVC.tableVCType isEqual:PAYMENT_PG_NET_BANKING]){
                PayUModelNetBanking *objNetBanking = [PayUModelNetBanking new];
                objNetBanking = [self.paymentRelatedDetail.netBankingArray objectAtIndex:_tableVC.selectedIndex];
                ...
                PayUModelStoredCard *storedCardObj = [PayUModelStoredCard new];
                storedCardObj = [self.paymentRelatedDetail.oneTapStoredCardArray objectAtIndex:_tableVC.selectedIndex];
                self.txtFieldVar2.text = storedCardObj.cardToken;
            }
        }
    }
        ...
        [_webServiceResponse getUserCards:self.paymentParam withCompletionBlock:^(NSDictionary *dictStoredCard, NSString *errorMessage, id extraParam) {
            ...
            if (!errorMessage) {
                ...
                for (NSString *cardtoken in [dictStoredCard allKeys]) {
                    ...
                    PayUModelStoredCard *objSC = [[PayUModelStoredCard alloc] init];
                    objSC = [dictStoredCard objectForKey:cardtoken];
