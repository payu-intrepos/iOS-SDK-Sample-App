...
class PUSACCDCVC: UIViewController,UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    params.enableVerbose = true
    params.standingInstruction = true
  ...
  func getCalculatedIntegerFrom(strings: [String]) -> Int {
        ...
        
    {
        ...
        let cardFrame = (self.lblEnterCardNumber.frame.height) + (self.txtEnterCardNumber.frame.height) + (self.lblExpiryMonth.frame.height) + (self.lblExpiryMonth.frame.height)
        
        
        
        let xCal = self.lblExpiryMonth.frame.origin.x + self.ccdcContainer1.frame.origin.x
        
        let yCal = self.view.frame.height - (self.ccdcContainer1.frame.height + self.ccdcContainer2.frame.height) + cardFrame
        
        
        
        let cardFrame2=self.lblEnterCardNumber.frame.height + self.txtEnterCardNumber.frame.height + self.lblExpiryYear.frame.height + self.lblExpiryYear.frame.height
        
        
        
        tblExpiryMonth = UITableView(frame: CGRect(x:xCal , y:yCal    , width: self.lblExpiryMonth.frame.width
            
            , height: (displayHeight - barHeight) - 64))
        
        let y2Cal = self.view.frame.height - (self.ccdcContainer1.frame.height + self.ccdcContainer2.frame.height) + cardFrame2
        
        let xCont = self.ccdcContainer1.frame.origin.x + self.lblExpiryYear.frame.origin.x
        
        tblExpiryYear = UITableView(frame: CGRect(x:xCont, y: y2Cal , width: self.lblExpiryYear.frame.width, height: (displayHeight - barHeight) - 64))
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if (tableView == tblExpiryMonth)
    {
      self.imageMonth.isHidden = true
        lblExpiryMonth.setTitle(cell?.textLabel?.text, for: UIControl.State.normal)
    ...
    if (tableView == tblExpiryYear)
    {
      self.imageYear.isHidden = true
        lblExpiryYear.setTitle(cell?.textLabel?.text, for: UIControl.State.normal)
